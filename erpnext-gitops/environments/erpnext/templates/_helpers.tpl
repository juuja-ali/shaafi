...
{{/*
ERPNext image reference with safety guards:
- lowercases the repository (OCI spec requires lowercase)
- falls back to "latest" when tag is empty or invalid (e.g. ends with "-")
Prevents InvalidImageName in Kubernetes.
*/}}
{{- define "erpnext.image" -}}
{{- $repo := .Values.image.repository | lower | trim -}}
{{- $tag := .Values.image.tag | default "latest" | trim -}}
{{- if or (not $tag) (hasSuffix "-" $tag) -}}
{{ $repo }}:latest
{{- else -}}
{{ $repo }}:{{ $tag }}
{{- end -}}
